...
import cv2
import numpy as np
from numpy import amax, amin, cos, sin
...
def estimate_skew_angle(raw, angleRange=[-15, 15]):
    """
    Estimated image text deflection angle,
    AngleRange: angle estimation interval
    """
...
def minAreaRect(coords):
    """
    rectangle circumscribed polygon
    """
...
def point_line_cor(p, A, B, C):
    """
    Consider the relationship between points and General line equation (Ax+By+c)=0
    """
    x, y = p
    r = A * x + B * y + C
    return r
...
def solve(box):
    """
    Xoay tọa độ góc quanh điểm cx, cy w, h
    x = cx-w/2
    y = cy-h/2
...
def minAreaRectbox(regions, flag=True, W=0, H=0, filtersmall=False, adjustBox=False):
    """
    đa giác ngoại tiếp hình chữ nhật
    """
    boxes = []
    for region in regions:
        ...
        if adjustBox:
            x1, y1, x2, y2, x3, y3, x4, y4 = xy_rotate_box(
                cx, cy, w + 5, h + 5, angle=0, degree=None
            )
...
def draw_boxes(im, bboxes, color=(0, 0, 0)):
    ...
    for box in bboxes:
        ...
        cv2.line(
            tmp, (int(x1), int(y1)), (int(x2), int(y2)), c, 1, lineType=cv2.LINE_AA
        )
        cv2.line(
            tmp, (int(x2), int(y2)), (int(x3), int(y3)), c, 1, lineType=cv2.LINE_AA
        )
        cv2.line(
            tmp, (int(x3), int(y3)), (int(x4), int(y4)), c, 1, lineType=cv2.LINE_AA
        )
        cv2.line(
            tmp, (int(x4), int(y4)), (int(x1), int(y1)), c, 1, lineType=cv2.LINE_AA
        )
...
def draw_boxes_v2(im, bboxes, color=(150, 244, 0)):
    ...
    for box in bboxes:
        ...
        cv2.line(
            tmp, (int(x1), int(y1)), (int(x2), int(y2)), c, 2, lineType=cv2.LINE_AA
        )
        cv2.line(
            tmp, (int(x2), int(y2)), (int(x3), int(y3)), c, 2, lineType=cv2.LINE_AA
        )
        cv2.line(
            tmp, (int(x3), int(y3)), (int(x4), int(y4)), c, 2, lineType=cv2.LINE_AA
        )
        cv2.line(
            tmp, (int(x4), int(y4)), (int(x1), int(y1)), c, 2, lineType=cv2.LINE_AA
        )
        ...
        border = cv2.copyMakeBorder(
            sub_img, 2, 2, 2, 2, cv2.BORDER_CONSTANT, value=[255, 255]
        )
